  ...
  for (var j = 0; j < block.height; j++) {
    for (var i = 0; i < block.width; i++) {
      ...
      if (Math.random() > 0.5) {
        ...
        block.pixels[blockPixelOffset] = Math.round(Math.max(0, Math.min(255, r0 + Math.random() * 1)));
        block.pixels[blockPixelOffset + 1] = Math.round(Math.max(0, Math.min(255, g0 + Math.random() * 1)));
        //block.pixels[blockPixelOffset + 2] = Math.round(Math.max(0, Math.min(255, b0 - Math.random() * 1)));
      }
    }
  }
...
function valuePerBlock(block) {
  ...
  block.totalg = totalg;
  block.totalb = totalb;
  var sensitivity = supportedAlphabet.length * 100;
  var total = Math.floor(Math.abs(totalr + totalb + totalg) / sensitivity);
  var value = total % supportedAlphabet.length;
  return value;
}
